...
class vip_pay_details(osv.osv_memory):
    _name = 'vip.pay.details'
    _description = 'Sales Details'

    _columns = {
        'date_start': fields.date(u'开始日期', required=True),
        'date_end': fields.date(u'结束日期', required=True),
        'user_ids': fields.many2many('res.users', 'vip_pay_details_report_user_rel', 'user_id', 'wizard_id', u'操作员'),
    }
    _defaults = {
